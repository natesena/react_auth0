...
class PreviewPost extends React.Component {
  ...
  componentDidMount() {
    let newTitleEditorState = EditorState.createWithContent(
      convertFromRaw(JSON.parse(this.props.title_es))
    );
    let newPreviewEditorState = EditorState.createWithContent(
      convertFromRaw(JSON.parse(this.props.preview_bes))
    );
    this.setState({
          ...
          <div>
            <div className={"preview-post-title"}>
              <Link
                ...
                className={"preview-post-title-link"}
              >
                <Editor
                  toolbarHidden={true}
